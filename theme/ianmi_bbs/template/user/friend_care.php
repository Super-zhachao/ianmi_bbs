...
<div class="tab ">
  ...
  <div class="tab-content">
    <div class="tab-page tab1 tab-active">
        <?php if ($care_list->toArray()['total']== 0) {
            useComp('/components/common/nofind', ['desc' => "这个地方空空如也,赶紧去关注感兴趣的人吧！"]);
        } else { ?>
          <div class="list">
                ...
                <div class="list-group friend_list">
                  <a href="/user/show?id=<?= $userinfo['id'] ?>" class="list-item list-item-image-text border-b">
                    ...
                    <div class="list-item-text">
                      ...
                      <?php if ($is_care) { ?>
                        <button data-id="<?= $userinfo['id'] ?>" data-href="/user/care_user?id=<?= $userinfo['id'] ?>"
                                class="btn  btn-sm btn_care">已关注
                        </button>
                      <?php } else { ?>
                        <button data-id="<?= $userinfo['id'] ?>" data-href="/user/care_user?id=<?= $userinfo['id'] ?>"
                                class="btn btn-shadow btn-fill btn-sm btn_care">关注
                        </button>
                      <?php } ?>
                  </a>
                </div>
